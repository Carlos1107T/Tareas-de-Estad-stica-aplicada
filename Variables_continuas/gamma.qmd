...
desv_std_poblacional <- sqrt(varianza_poblacional)

cat("=== PARÁMETROS TEÓRICOS GAMMA ===\n")
cat("Alpha (shape):", alpha_tcl, "\n")
cat("Lambda (rate):", lambda_tcl, "\n")
cat("Media teórica:", round(media_poblacional, 4), "\n")
cat("Varianza teórica:", round(varianza_poblacional, 4), "\n")
cat("Desviación estándar teórica:", round(desv_std_poblacional, 4), "\n\n")


muestra_grande <- rgamma(8000, shape = alpha_tcl, rate = lambda_tcl)


p_poblacion <- ggplot(data.frame(x = muestra_grande), aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50,
                   fill = "lightblue", color = "black", alpha = 0.7) +
    stat_function(fun = dgamma, args = list(shape = alpha_tcl, rate = lambda_tcl),
                  color = "red", linewidth = 1.2) +
    geom_vline(xintercept = media_poblacional, color = "red", linetype = "dashed", linewidth = 1) +
    geom_vline(xintercept = median(muestra_grande), color = "blue", linetype = "dotted", linewidth = 1) +
    labs(title = "Distribución Gamma Poblacional",
         subtitle = paste("Gamma(α =", alpha_tcl, ", λ =", lambda_tcl, ")"),
         x = "Valores",
         y = "Densidad",
         caption = "Curva roja: Densidad teórica | Línea roja: Media teórica | Línea azul: Mediana muestral") +
    theme_minimal() +
...
for (i in seq_along(size_muestra)) {
    n <- size_muestra[i]
    cat("Generando", num_muestras, "medias con n =", n, "...\n")
    ...
    sesgo_medias <- moments::skewness(medias)
    
    
    resultados_tcl[[as.character(n)]] <- medias
    ...
    cat("  n =", n, "| Media muestral:", round(media_de_medias, 4), 
        "| Media teórica:", round(media_poblacional, 4), 
        "| Diferencia:", round(abs(media_de_medias - media_poblacional), 4), "\n")
    cat("  n =", n, "| Varianza muestral:", round(varianza_de_medias, 4),
        "| Varianza teórica/n:", round(varianza_poblacional/n, 4),
        "| Diferencia:", round(abs(varianza_de_medias - varianza_poblacional/n), 4), "\n")
    cat("  n =", n, "| Sesgo:", round(sesgo_medias, 4), "\n\n")
}


df_tcl$n_factor <- factor(df_tcl$n_factor, 
...
p_tcl <- ggplot(df_tcl, aes(x = medias)) +
    ...
    labs(title = "Teorema Central del Límite - Distribución Gamma",
         subtitle = paste("Distribución de medias muestrales para Gamma(α =", alpha_tcl, ", λ =", lambda_tcl, ")"),
         x = "Media muestral",
         y = "Densidad",
         caption = "Línea roja: Media teórica | Curva roja: Normal teórica según TCL") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5))
...
cat("=== ANÁLISIS DE CONVERGENCIA A LA NORMALIDAD ===\n")

resultados_convergencia <- data.frame()

for (n in size_muestra) {
    medias_n <- resultados_tcl[[as.character(n)]]
    
    
    skewness_val <- moments::skewness(medias_n)
    kurtosis_val <- moments::kurtosis(medias_n)
    
   
    if (n >= 20) {
        shapiro_test <- shapiro.test(medias_n)
        p_value <- shapiro_test$p.value
        normal_shapiro <- p_value > 0.05
    } else {
        p_value <- NA
        normal_shapiro <- NA
    }
    
    # Criterios de normalidad aproximada
    normal_skew <- abs(skewness_val) < 0.5
    normal_kurt <- abs(kurtosis_val - 3) < 1
    
    df_temp <- data.frame(
        n = n,
        skewness = skewness_val,
        kurtosis = kurtosis_val,
        p_value_shapiro = p_value,
        normal_aproximada = normal_skew & normal_kurt
    )
    
    resultados_convergencia <- rbind(resultados_convergencia, df_temp)
    
    cat("n =", n, "| Skewness:", round(skewness_val, 4), 
        "| Kurtosis:", round(kurtosis_val, 4))
    
    if (!is.na(p_value)) {
        cat(" | Shapiro p-value:", round(p_value, 4))
    }
    
    if (normal_skew & normal_kurt) {
        cat(" → APROX NORMAL\n")
    } else {
        cat(" → NO NORMAL\n")
    }
...
# 7. Resumen final
cat("\n=== RESUMEN FINAL - VERIFICACIÓN TCL ===\n")
cat("Distribución poblacional: Gamma(α =", alpha_tcl, ", λ =", lambda_tcl, ")\n")
cat("Media poblacional:", round(media_poblacional, 4), "\n")
cat("Varianza poblacional:", round(varianza_poblacional, 4), "\n")
cat("Sesgo poblacional teórico:", round(2/sqrt(alpha_tcl), 4), "\n\n")

cat("El TCL establece que:\n")
cat("1. E[media_muestral] = μ =", round(media_poblacional, 4), "\n")
cat("2. Var[media_muestral] = σ²/n =", round(varianza_poblacional, 4), "/n\n")
cat("3. La distribución de medias se aproxima a N(μ, σ²/n) cuando n → ∞\n\n")

cat("CONCLUSIÓN:\n")
cat("Se confirma el TCL:\n")
cat("- Las medias muestrales se acercan a la media poblacional\n")
cat("- La varianza de medias decrece como σ²/n\n") 
cat("- La distribución se vuelve aproximadamente normal para n ≥ 30\n")
```
...
simular_sumas_exponenciales <- function(n, lambda, n_sim) {
 
  sumas <- replicate(n_sim, sum(rexp(n, rate = lambda)))
  

  ggplot(data.frame(sumas = sumas), aes(x = sumas)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30,
                   fill = "lightblue", color = "black", alpha = 0.7) +
    stat_function(fun = dgamma, 
                  args = list(shape = n, rate = lambda),
                  color = "red", size = 1.2) +
    labs(title = paste("Suma de", n, "Exponenciales(λ =", lambda, ")"),
         subtitle = paste("Histograma: Simulación | Curva roja: Gamma(", n, ",", lambda, ")"),
         x = "Suma",
         y = "Densidad") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5),
...
for (n in valores_n) {
  print(simular_sumas_exponenciales(n, lambda, n_simulaciones))
}
```
