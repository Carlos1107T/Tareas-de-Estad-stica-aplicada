...
cat("=== PROPIEDAD (a): t(n) → F(1,n) ===\n")
n <- 10
set.seed(123)


t_muestras <- rt(n_muestras, df = n)
t_cuadrado <- t_muestras^2
...
p_a <- ggplot(df_a, aes(x = valor)) +
  ...
  geom_function(fun = function(x) df(x, 1, n), color = "red", linewidth = 1.2) +
  xlim(0, 15) +
  labs(title = "Propiedad (a): Si X ~ t(n) entonces X² ~ F(1,n)",
       subtitle = paste("n =", n),
       x = "Valores",
       y = "Densidad") +
  theme_minimal() +
...
# Prueba KS para propiedad (a)
ks_test_a <- ks.test(t_cuadrado, "pf", df1 = 1, df2 = n)
cat("Prueba KS - p-value:", round(ks_test_a$p.value, 4), "\n")
if(ks_test_a$p.value > 0.05) {
  cat("Se verifica la propiedad (a)\n\n")
} else {
  cat("No se verifica la propiedad (a)\n\n")
}

# b. Si X ~ F(a,b) entonces 1/X ~ F(b,a)
cat("=== PROPIEDAD (b): F(a,b) → F(b,a) ===\n")
a <- 5
b <- 8


f_muestras <- rf(n_muestras, df1 = a, df2 = b)
f_inversa <- 1 / f_muestras
...
p_b <- ggplot(df_b, aes(x = valor)) +
  ...
  geom_function(fun = function(x) df(x, b, a), color = "red", linewidth = 1.2) +
  xlim(0, 10) +
  labs(title = "Propiedad (b): Si X ~ F(a,b) entonces 1/X ~ F(b,a)",
       subtitle = paste("a =", a, ", b =", b),
       x = "Valores",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
# Prueba KS para propiedad (b)
ks_test_b <- ks.test(f_inversa, "pf", df1 = b, df2 = a)
cat("Prueba KS - p-value:", round(ks_test_b$p.value, 4), "\n")
if(ks_test_b$p.value > 0.05) {
  cat("Se verifica la propiedad (b)\n\n")
} else {
  cat("No se verifica la propiedad (b)\n\n")
}

# c. Si X ~ χ²(a) y Y ~ χ²(b) entonces (X/a)/(Y/b) ~ F(a,b)
cat("=== PROPIEDAD (c): (χ²(a)/a)/(χ²(b)/b) → F(a,b) ===\n")
a <- 5
b <- 10

df_expr_c <- tibble(
    Muestra = 1:n_muestras,
    Jia = rchisq(n_muestras, df = a),
    Ji_b = rchisq(n_muestras, df = b),
    F_simulada = (Jia/a)/(Ji_b/b)
)

p_c <- ggplot(df_expr_c) + 
  geom_histogram(aes(x = F_simulada, y = after_stat(density)), bins = 100, 
                 fill = 'lightcoral', color = 'black', alpha = 0.7) +
  geom_function(fun = function(x) df(x, a, b), color = "red", linewidth = 1.2) +
  xlim(0, 7.5) +
  labs(title = "Propiedad (c): (χ²(a)/a)/(χ²(b)/b) ~ F(a,b)",
       subtitle = paste("a =", a, ", b =", b),
       x = "Valores",
       y = "Densidad") +
  theme_minimal() +
...
# Prueba KS para propiedad (c)
ks_test_c <- ks.test(df_expr_c$F_simulada, "pf", df1 = a, df2 = b)
cat("Prueba KS - p-value:", round(ks_test_c$p.value, 4), "\n")
if(ks_test_c$p.value > 0.05) {
  cat("Se verifica la propiedad (c)\n")
} else {
  cat("No se verifica la propiedad (c)\n")
}
