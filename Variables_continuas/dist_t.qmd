...
media_teorica <- 0
varianza_teorica <- gl / (gl - 2)  

cat("=== PARÁMETROS TEÓRICOS t(", gl, ") ===\n")
...
for (n in tamanos_muestra) {
  ...
  cat("  Media muestral:", round(media_muestral, 4), 
      "| Diferencia:", round(abs(media_muestral - media_teorica), 4), "\n")
  cat("  Varianza muestral:", round(varianza_muestral, 4),
      "| Diferencia:", round(abs(varianza_muestral - varianza_teorica), 4), "\n\n")
}


muestra_grande <- rt(1000, df = gl)
...
ggplot(data.frame(x = muestra_grande), aes(x = x)) +
  ...
  labs(title = paste("Distribución t de Student -", gl, "grados de libertad"),
       subtitle = "n = 1000 observaciones",
       x = "Valores",
       y = "Densidad",
       caption = "Curva roja: t(5) teórica | Curva azul: Normal(0,1)") +
  xlim(-4, 4) +
  theme_minimal() +
...
ggplot(df_exr)+
  ...
  geom_function(fun = function(x) dt(x, df=n), aes(color = "Dist t"), linewidth=1.3)+
  geom_function(fun = function(x) dnorm(x), aes(color = "Dist Z"), linewidth=0.5)+
  scale_color_manual(name = "Dist", values =c("Dist t"="red", "Dist Z"="yellow"))+
  theme_bw()
...
T_calculada <- X / sqrt(Y/gl)

ggplot(data.frame(T = T_calculada), aes(x = T)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "lightblue", color = "black", alpha = 0.7) +
  stat_function(fun = dt, args = list(df = gl),
                color = "red", linewidth = 1.2) +
  labs(title = "Distribución t de Student - Comparación",
       subtitle = paste("T = Z / √(χ²(", gl, ")/", gl, ") vs t(", gl, ") teórica"),
       x = "Valores de T",
       y = "Densidad",
       caption = "Histograma: T calculada | Curva roja: t(10) teórica") +
  xlim(-4, 4) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
cat("T CALCULADA (Z / √(χ²/gl)):\n")
cat("Media:", round(mean(T_calculada), 4), "| Teórica: 0\n")
cat("Varianza:", round(var(T_calculada), 4), "| Teórica:", round(gl/(gl-2), 4), "\n\n")


ks_test <- ks.test(T_calculada, "pt", df = gl)
...
valores <- replicate(N_muestras, media_sd(n_muestras, mu, sigma))

df_exr2 <- tibble(
  Muestra = 1:N_muestras,
  Media = valores[1,],
  SD = valores[2,],
  T = (Media - mu)/(SD/sqrt(n_muestras))
)

ggplot(df_exr2)+
  geom_histogram(aes(x=T, y = after_stat(density)), bins=100, fill ="lightgreen", color = "black", alpha = 0.7)+
  geom_function(fun = function(x) dt(x, df=n_muestras-1), color = "cyan", linewidth =1.2)+
  theme_bw()
```
