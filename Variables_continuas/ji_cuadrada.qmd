...
pdf_vals <- dchisq(x, df = n_ejemplo)
cdf_vals <- pchisq(x, df = n_ejemplo) * 0.5

df <- data.frame(x = x, pdf = pdf_vals, cdf = cdf_vals)

# Gráfico básico sin elementos problemáticos
ggplot(df, aes(x = x)) +
  geom_ribbon(aes(ymin = 0, ymax = ifelse(x <= 8, pdf, 0)), 
              fill = "lightblue", alpha = 0.7) +
  geom_line(aes(y = pdf, color = "PDF"), size = 1.2) +
  geom_line(aes(y = cdf, color = "CDF × 0.5"), size = 1.2) +
  geom_vline(xintercept = 8, linetype = "dashed", color = "red") +
  scale_color_manual(values = c("PDF" = "#d62728", "CDF × 0.5" = "#1f7764")) +
  labs(title = "PDF vs CDF (Chi-cuadrado, n=5)",
       x = "x", y = "Densidad") +
  theme_minimal()
...
p_media <- ggplot(resultados, aes(x = n, y = media_muestral, color = factor(gl))) +
  geom_line(linewidth = 1) +
  geom_hline(aes(yintercept = media_teorica, color = factor(gl)), 
             linetype = "dashed", linewidth = 1) +
  labs(title = "Convergencia de la Media Muestral",
       subtitle = "Distribución Ji-cuadrada",
       x = "Tamaño de Muestra",
       y = "Media Muestral",
...
p_varianza <- ggplot(resultados, aes(x = n, y = varianza_muestral, color = factor(gl))) +
  geom_line(linewidth = 1) +
  geom_hline(aes(yintercept = varianza_teorica, color = factor(gl)), 
             linetype = "dashed", linewidth = 1) +
...
for (gl in grados_libertad) {
  muestra_grande <- rchisq(10000, df = gl)
  
  p_hist <- ggplot(data.frame(x = muestra_grande), aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 40,
                   fill = "lightblue", color = "black", alpha = 0.7) +
    stat_function(fun = dchisq, args = list(df = gl),
                  color = "red", linewidth = 1.2) +
    labs(title = paste("Distribución Ji-cuadrada -", gl, "grados de libertad"),
...
media_poblacional <- gl
varianza_poblacional <- 2 * gl
...
for (n in size_muestra) {
  medias <- replicate(num_muestras, mean(rchisq(n, df = gl)))
  ...
  df_tcl <- rbind(df_tcl, df_temp)
}


df_tcl$n_factor <- factor(df_tcl$n_factor, levels = paste("n =", size_muestra))
...
ggplot(df_tcl, aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "lightblue", color = "black", alpha = 0.7) +
  geom_vline(xintercept = media_poblacional, color = "red", linetype = "dashed", linewidth = 1) +
  geom_line(data = df_normal_teorica, aes(x = x, y = y),
            color = "red", linewidth = 1.2, alpha = 0.8) +
  facet_wrap(~n_factor, scales = "free_y", ncol = 3) +
  labs(title = "Teorema Central del Límite - Distribución Ji-cuadrada",
       subtitle = paste("Distribución de medias muestrales para χ²(", gl, ")"),
       x = "Media muestral",
       y = "Densidad",
       caption = paste("Línea roja: Media teórica =", media_poblacional, 
                      "| Curva roja: Normal teórica según TCL")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
```
...
set.seed(369)
sumas_cuadrados <- numeric(n_simulaciones)

for(i in 1:n_simulaciones) {
  normales <- rnorm(gl_total)
  suma_cuadrados <- sum(normales^2)
  sumas_cuadrados[i] <- suma_cuadrados
}


ji_cuadrada_directo <- rchisq(n_simulaciones, df = gl_total)

df_suma <- data.frame(valor = sumas_cuadrados, metodo = "Suma de Z²")
df_directo <- data.frame(valor = ji_cuadrada_directo, metodo = "χ² directo")
df_combinado <- rbind(df_suma, df_directo)


ggplot(df_combinado, aes(x = valor, fill = metodo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, alpha = 0.6, position = "identity") +
  stat_function(fun = dchisq, args = list(df = gl_total), 
                color = "red", linewidth = 1.2) +
  labs(title = "Propiedad de Aditividad de la Ji-cuadrada",
       subtitle = paste("Comparación: Suma de", gl_total, "Z² vs χ²(", gl_total, ") directa"),
       x = "Valores",
       y = "Densidad",
       fill = "Método") +
  theme_minimal() +
...
cat("Grados de libertad:", gl_total, "\n\n")

cat("SUMA DE NORMALES ESTÁNDAR AL CUADRADO:\n")
cat("Media:", round(mean(sumas_cuadrados), 4), "| Teórica:", gl_total, "\n")
cat("Varianza:", round(var(sumas_cuadrados), 4), "| Teórica:", 2*gl_total, "\n\n")

cat("DISTRIBUCIÓN χ² DIRECTA:\n")
cat("Media:", round(mean(ji_cuadrada_directo), 4), "| Teórica:", gl_total, "\n")
cat("Varianza:", round(var(ji_cuadrada_directo), 4), "| Teórica:", 2*gl_total, "\n\n")


ks_test <- ks.test(sumas_cuadrados, ji_cuadrada_directo)
cat("PRUEBA KOLMOGOROV-SMIRNOV:\n")
cat("Estadístico D:", round(ks_test$statistic, 4), "\n")
cat("p-value:", round(ks_test$p.value, 4), "\n")
if(ks_test$p.value > 0.05) {
  cat("No hay evidencia para rechazar que las distribuciones son iguales\n")
} else {
...
set.seed(369)
for (gl in grados_libertad) {
  muestras <- rchisq(n_simulaciones, df = gl)
  
  media_teorica <- gl
  varianza_teorica <- 2 * gl
  
  
  muestras_estandarizadas <- (muestras - media_teorica) / sqrt(varianza_teorica)
  
  df_temp <- data.frame(
    valor = muestras_estandarizadas,
    gl = gl,
    gl_factor = paste("gl =", gl)
  )
...
df_resultados$gl_factor <- factor(df_resultados$gl_factor, 
                                 levels = paste("gl =", grados_libertad))


ggplot(df_resultados, aes(x = valor)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "lightblue", color = "black", alpha = 0.7) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1),
                color = "red", linewidth = 1.2) +
  facet_wrap(~gl_factor, scales = "free", ncol = 2) +
  labs(title = "Aproximación Normal de la Distribución Ji-cuadrada",
       subtitle = "Muestras estandarizadas vs Normal(0,1)",
       x = "Valores Estandarizados",
       y = "Densidad",
       caption = "Curva roja: Distribución Normal Estándar") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))


# Teoría: Para gl grande, χ²(gl) ≈ N(gl, 2gl)
cat("\n=== EXPLICACIÓN TEÓRICA ===\n")
cat("Para grados de libertad grandes (gl > 30), la distribución ji-cuadrada\n")
cat("se puede aproximar por una distribución normal con:\n")
cat("Media: gl\n")
cat("Varianza: 2gl\n")
cat("La aproximación mejora a medida que gl aumenta.\n")
```
...
# Método 1: Suma de dos ji-cuadradas independientes
suma_ji_cuadradas <- numeric(n_simulaciones)
for(i in 1:n_simulaciones) {
  x1 <- rchisq(1, df = gl1)
  x2 <- rchisq(1, df = gl2)
  suma_ji_cuadradas[i] <- x1 + x2
}
...
df_suma <- data.frame(valor = suma_ji_cuadradas, metodo = paste("χ²(", gl1, ") + χ²(", gl2, ")"))
df_directo <- data.frame(valor = ji_cuadrada_directo, metodo = paste("χ²(", gl_total, ") directo"))
df_combinado <- rbind(df_suma, df_directo)
...
ggplot(df_combinado, aes(x = valor, fill = metodo)) +
  ...
  stat_function(fun = dchisq, args = list(df = gl_total), 
                color = "red", linewidth = 1.2) +
  labs(title = "Propiedad de Aditividad de la Ji-cuadrada",
       subtitle = paste("Comparación: χ²(", gl1, ") + χ²(", gl2, ") vs χ²(", gl_total, ") directa"),
       x = "Valores",
       y = "Densidad",
       fill = "Método") +
  theme_minimal() +
...
cat("χ²(", gl1, ") + χ²(", gl2, ") = χ²(", gl_total, ")\n\n")

cat("SUMA DE JI-CUADRADAS INDEPENDIENTES:\n")
cat("Media:", round(mean(suma_ji_cuadradas), 4), "| Teórica:", gl_total, "\n")
cat("Varianza:", round(var(suma_ji_cuadradas), 4), "| Teórica:", 2*gl_total, "\n\n")
...
ks_test <- ks.test(suma_ji_cuadradas, ji_cuadrada_directo)
cat("PRUEBA KOLMOGOROV-SMIRNOV:\n")
cat("Estadístico D:", round(ks_test$statistic, 4), "\n")
cat("p-value:", round(ks_test$p.value, 4), "\n")
if(ks_test$p.value > 0.05) {
  cat("No hay evidencia para rechazar que las distribuciones son iguales\n")
  cat("→ Se verifica la propiedad de aditividad\n")
} else {
  cat("Las distribuciones son significativamente diferentes\n")
  cat("→ No se verifica la propiedad de aditividad\n")
}
```
