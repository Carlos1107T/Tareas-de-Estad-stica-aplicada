...
La **distribución Weibull** es una distribución de probabilidad continua ampliamente utilizada en estudios de confiabilidad, análisis de supervivencia y teoría de valores extremos. Debe su nombre al matemático e ingeniero sueco Ernst Hjalmar Waloddi Weibull (1887-1979), quien la popularizó.

## 1.1 Propiedades de la Función de Densidad de Probabilidad (PDF)
...
## 1.2 Función de Distribución (CDF, Probabilidad Acumulada)
...
ggplot(df_cdf, aes(x = x, y = y, color = as.factor(alpha), linetype = as.factor(lambda))) +
  ...
  annotate("text", x = 2.5, y = 0.55, label = "Mediana", color = "gray")
```
...
## 1.3 Parámetros y Medidas Descriptivas
...
## 1.4 Simulación y Comparación con Parámetros Teóricos
...
ggplot(data.frame(tiempo = tiempo, supervivencia = supervivencia), 
  ...
  geom_hline(yintercept = supervivencia_1_ano, linetype = "dashed", color = "red") +
  annotate("point", x = 365, y = supervivencia_1_ano, color = "red", size = 3) +
...
ggplot(data.frame(velocidad = velocidad_viento), aes(x = velocidad)) +
  ...
  ), aes(x = x, y = y), color = "darkgreen", linewidth = 1) +
...
## 1.6 Verificación del Teorema Central del Límite
...
### Resumen Visual del Ejercicio 2
...
ggplot(comparacion, aes(x = x)) +
  ...
  scale_color_manual(values = c("Fórmula Demostrada" = "blue", 
                               "Integración Numérica" = "red"))
```

## Conclusión

La distribución Weibull es una herramienta poderosa en el análisis de datos de confiabilidad y supervivencia. Su flexibilidad para modelar diferentes comportamientos de tasa de falla la hace invaluable en aplicaciones prácticas. El entendimiento de sus propiedades y la capacidad de simular y analizar datos Weibull son habilidades esenciales en estadística aplicada.