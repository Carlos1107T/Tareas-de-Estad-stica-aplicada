...
mu_altura <-170
sigma_altura  <-8
muestra_altura <- rnorm(500, mean = 170, sd = 8)
...
cat("La desviación de la altura es:", sd_altura, "\n")
...
ggplot(dist_altura)+
  ...
  annotate("label", x=190, y=0.01, 
           label = paste("1-F(180)= ",round( pnorm(180, mu_altura, sigma_altura, lower.tail = FALSE),4)), color ="gold3", size =5, fill= "yellow")+
  theme_bw()
```
...
fuera_especificacion <- sum(temperaturas < limite_inf | temperaturas > limite_sup)
porcentaje_fuera <- fuera_especificacion / n_mediciones * 100


mu_descentrado <- 355
temperaturas_descentrado <- rnorm(n_mediciones, mean = mu_descentrado, sd = sigma)
fuera_descentrado <- sum(temperaturas_descentrado < limite_inf | temperaturas_descentrado > limite_sup)
porcentaje_fuera_descentrado <- fuera_descentrado / n_mediciones * 100


prob_dentro <- 0.999
z_value <- qnorm((1 + prob_dentro) / 2)
sigma_necesario <- (limite_sup - mu) / z_value


cat("=== RESULTADOS CONTROL DE TEMPERATURA ===\n")
cat("1. Porcentaje fuera de especificación (μ=350):", round(porcentaje_fuera, 2), "%\n")
cat("2. Porcentaje fuera de especificación (μ=355):", round(porcentaje_fuera_descentrado, 2), "%\n")
cat("3. σ necesario para 99.9% dentro de especificación:", round(sigma_necesario, 4), "°C\n")

df_temperaturas <- data.frame(
  temperatura = temperaturas,
  proceso = "Centrado (μ=350)"
)

df_temperaturas_descentrado <- data.frame(
  temperatura = temperaturas_descentrado,
  proceso = "Descentrado (μ=355)"
)

df_combinado <- rbind(df_temperaturas, df_temperaturas_descentrado)

ggplot(df_combinado, aes(x = temperatura, fill = proceso)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40, alpha = 0.7, position = "identity") +
  geom_vline(xintercept = c(limite_inf, limite_sup), color = "red", linetype = "dashed", linewidth = 1) +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), 
                color = "blue", linewidth = 1, xlim = c(mu-3*sigma, mu+3*sigma)) +
  stat_function(fun = dnorm, args = list(mean = mu_descentrado, sd = sigma), 
                color = "darkorange", linewidth = 1, xlim = c(mu_descentrado-3*sigma, mu_descentrado+3*sigma)) +
  labs(title = "Control de Temperatura Industrial",
       subtitle = paste("Límites: ", limite_inf, "°C -", limite_sup, "°C"),
       x = "Temperatura (°C)",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
prob_60000 <- 1 - pnorm(60000, mean = mu, sd = sigma)


prob_30000 <- pnorm(30000, mean = mu, sd = sigma)
meses_esperados_30000 <- prob_30000 * n_meses


Q1_teorico <- qnorm(0.25, mean = mu, sd = sigma)
Q3_teorico <- qnorm(0.75, mean = mu, sd = sigma)
IQR_teorico <- Q3_teorico - Q1_teorico


prob_45000 <- 1 - pnorm(45000, mean = mu, sd = sigma)
porcentaje_45000 <- prob_45000 * 100


cat("=== ANÁLISIS DE VENTAS MENSUALES ===\n")
cat("1. Ventas simuladas de 24 meses:\n")
print(round(ventas, 0))
cat("\n2. Probabilidad ventas > 60,000:", round(prob_60000, 4), "(", round(prob_60000 * 100, 2), "%)\n")
cat("3. Meses esperados < 30,000:", round(meses_esperados_30000, 2), "\n")
cat("4. Rango intercuartílico teórico:", round(IQR_teorico, 2), "pesos\n")
cat("5. Porcentaje que supera 45,000:", round(porcentaje_45000, 2), "%\n")


df_ventas <- data.frame(mes = 1:n_meses, ventas = ventas)

ggplot(df_ventas, aes(x = ventas)) +
  geom_histogram(aes(y = after_stat(density)), bins = 8, 
                 fill = "lightblue", color = "black", alpha = 0.7) +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma),
                color = "red", linewidth = 1.2) +
  geom_vline(xintercept = c(30000, 45000, 60000), 
             color = c("red", "orange", "green"),
             linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Ventas Mensuales",
       subtitle = "N(50000, 12000²)",
       x = "Ventas (pesos)",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
set.seed(333)
n_baterias <- 800
vida_util <- rnorm(n_baterias, mean = mu, sd = sigma)
...
baterias_fallan <- prob_falla * 10000


media_simulada <- mean(vida_util)
mediana_simulada <- median(vida_util)

percentil_10 <- qnorm(0.10, mean = mu, sd = sigma)


cat("=== ANÁLISIS DE VIDA ÚTIL DE BATERÍAS ===\n")
cat("1. Simuladas", n_baterias, "baterías\n")
cat("2. Garantía que cubre 95%:", round(garantia_95, 2), "horas\n")
cat("3. Baterías que fallan (garantía 900h, 10,000 unidades):", round(baterias_fallan, 0), "\n")
cat("4. Media simulada:", round(media_simulada, 2), "| Media teórica:", mu, "\n")
cat("   Mediana simulada:", round(mediana_simulada, 2), "| Mediana teórica:", mu, "\n")
cat("5. Percentil 10:", round(percentil_10, 2), "horas\n")
...
set.seed(333)
n_mediciones <- 1000
errores <- rnorm(n_mediciones, mean = mu, sd = sigma)


porcentaje_5_5 <- pnorm(5, mu, sigma) - pnorm(-5, mu, sigma)
porcentaje_5_5_empirico <- sum(errores >= -5 & errores <= 5) / n_mediciones * 100


prob_abs_6 <- 2 * (1 - pnorm(6, mu, sigma))
prob_abs_6_empirico <- sum(abs(errores) > 6) / n_mediciones


porcentaje_preciso <- pnorm(3, mu, sigma) - pnorm(-3, mu, sigma)
porcentaje_preciso_empirico <- sum(abs(errores) <= 3) / n_mediciones * 100


media_errores <- mean(errores)
mediana_errores <- median(errores)
simetria_empirica <- abs(mean(errores[errores > 0])) - abs(mean(errores[errores < 0]))


cat("=== ANÁLISIS DE ERRORES DE MEDICIÓN ===\n")
cat("1. Simuladas", n_mediciones, "mediciones\n")
cat("2. Errores entre -5 y +5 unidades:\n")
cat("   Teórico:", round(porcentaje_5_5 * 100, 2), "%\n")
cat("   Empírico:", round(porcentaje_5_5_empirico, 2), "%\n")
cat("3. Probabilidad |error| > 6 unidades:\n")
cat("   Teórico:", round(prob_abs_6, 4), "\n")
cat("   Empírico:", round(prob_abs_6_empirico, 4), "\n")
cat("4. Mediciones precisas (|error| ≤ 3):\n")
cat("   Teórico:", round(porcentaje_preciso * 100, 2), "%\n")
cat("   Empírico:", round(porcentaje_preciso_empirico, 2), "%\n")
cat("5. Verificación simetría:\n")
cat("   Media empírica:", round(media_errores, 4), "\n")
cat("   Mediana empírica:", round(mediana_errores, 4), "\n")
cat("   Diferencia simetría:", round(simetria_empirica, 4), "\n")


df_errores <- data.frame(errores = errores)

ggplot(df_errores, aes(x = errores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 fill = "lightcoral", color = "black", alpha = 0.7) +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma),
                color = "red", linewidth = 1.2) +
  geom_vline(xintercept = 0, color = "blue", linetype = "solid", linewidth = 1) +
  geom_vline(xintercept = c(-5, 5, -6, 6, -3, 3), 
             color = c("green", "green", "orange", "orange", "purple", "purple"),
...
set.seed(369)
n_estudiantes <- 200
grupo_A <- rnorm(n_estudiantes, mean = mu_A, sd = sigma_A)
grupo_B <- rnorm(n_estudiantes, mean = mu_B, sd = sigma_B)


porcentaje_A_supera_B <- sum(grupo_A > mu_B) / n_estudiantes * 100
porcentaje_A_supera_B_teorico <- (1 - pnorm(mu_B, mu_A, sigma_A)) * 100


prob_B_menor_70 <- pnorm(70, mu_B, sigma_B)
prob_B_menor_70_empirico <- sum(grupo_B < 70) / n_estudiantes
...
cat("=== COMPARACIÓN DE PUNTUACIONES ===\n")
cat("1. Simulados 200 estudiantes por grupo\n")
cat("2. Grupo A supera media del Grupo B:\n")
cat("   Empírico:", round(porcentaje_A_supera_B, 2), "%\n")
cat("   Teórico:", round(porcentaje_A_supera_B_teorico, 2), "%\n")
cat("3. Probabilidad Grupo B < 70:\n")
cat("   Teórico:", round(prob_B_menor_70, 4), "\n")
cat("   Empírico:", round(prob_B_menor_70_empirico, 4), "\n")
cat("4. Percentil 90 (10% superior):\n")
cat("   Grupo A:", round(percentil_90_A, 2), "\n")
cat("   Grupo B:", round(percentil_90_B, 2), "\n")
...
ggplot(df_grupos, aes(x = puntuacion, fill = grupo)) +
  geom_histogram(aes(y = after_stat(density)), alpha = 0.6, bins = 30, 
                 position = "identity") +
  stat_function(fun = dnorm, args = list(mean = mu_A, sd = sigma_A),
                color = "blue", linewidth = 1, xlim = c(mu_A-3*sigma_A, mu_A+3*sigma_A)) +
  stat_function(fun = dnorm, args = list(mean = mu_B, sd = sigma_B),
                color = "red", linewidth = 1, xlim = c(mu_B-3*sigma_B, mu_B+3*sigma_B)) +
  geom_vline(xintercept = c(mu_A, mu_B), 
             color = c("blue", "red"), linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = c(percentil_90_A, percentil_90_B),
             color = c("blue", "red"), linetype = "dotted", linewidth = 0.8) +
