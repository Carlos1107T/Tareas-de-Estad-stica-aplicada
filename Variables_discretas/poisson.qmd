...
ggplot(data_suma_comp, aes(x = valores, fill = metodo)) +
  ...
  theme_minimal() +
  theme(legend.position = "top") +
  scale_fill_manual(values = c("X₁ + X₂" = "#3498DB", "Poisson(λ₁+λ₂)" = "#E74C3C"))
```
...
# Intervalos de confianza usando aproximación normal (λ > 10)
if(lambda_total >= 10) {
  ic_inferior <- lambda_total - 1.96 * sqrt(lambda_total)
  ic_superior <- lambda_total + 1.96 * sqrt(lambda_total)
  cat("   IC 95%: [", round(ic_inferior, 1), ",", round(ic_superior, 1), "] defectos\n")
}
```
...
```{r}
# Parámetro lambda para diferentes intervalos
lambda_minuto <- 6
lambda_30seg <- 6 * 0.5  
lambda_hora <- 6 * 60    
lambda_2min <- 6 * 2    
...
cat("a) P(X = 4 llamadas en 1 minuto) =", round(prob_4_llamadas, 4), "\n")
```
...
cat("b) P(X = 0 llamadas en 30 segundos) =", round(prob_0_30seg, 4), "\n")
```
...
cat("c) Número esperado de llamadas en una hora:", lambda_hora, "\n")
```
...
```{r}
prob_8_a_12 <- sum(dpois(8:12, lambda_2min))
cat("d) P(8 ≤ X ≤ 12 llamadas en 2 minutos) =", round(prob_8_a_12, 4), "\n")
```


```{r}

data_llamadas <- data.frame(k = k_vals, probabilidad = prob_vals)


data_llamadas$color <- "Normal"
data_llamadas$color[data_llamadas$k == 4] <- "P(X = 4)"
data_llamadas$color[data_llamadas$k == 0] <- "P(X = 0)"


ggplot(data_llamadas, aes(x = k, y = probabilidad, fill = color)) +
    geom_col(alpha = 0.8, width = 0.7) +
    labs(title = "Distribución de Poisson: Llamadas Telefónicas",
...
lambda_500 <- lambda_base * (productos / 100)
prob_8_defectos <- dpois(8, lambda_500)
cat("a) P(X = 8 defectos en 500 productos) =", round(prob_8_defectos, 4), "\n")
```
...
cat("b) P(X ≥ 10 defectos en 500 productos) =", round(prob_10_mas, 4), "\n")
```
...
# P(X ≥ 1) = 1 - P(X = 0) = 1 - e^(-λ) = 0.95
# e^(-λ) = 0.05 => λ = -ln(0.05)
lambda_deseado <- -log(0.05)
# λ = 1.5 * (n/100) => n = (λ * 100) / 1.5
n_deseado <- (lambda_deseado * 100) / lambda_base

cat("c) Productos para P(≥1 defecto) = 0.95:", round(n_deseado, 0), "\n")
```
...
lambda_mejorado <- 1.2 * (productos / 100)
prob_10_mas_mejorado <- 1 - ppois(9, lambda_mejorado)

cat("d) P(X ≥ 10 defectos con mejora) =", round(prob_10_mas_mejorado, 4), "\n")
```
...
cat("a) P(X = 3 pacientes en 1 hora) =", round(prob_3_pacientes, 4), "\n")
...
cat("b) P(X = 0 pacientes en 2 horas) =", round(prob_0_2horas, 4), "\n")
...
prob_superar_capacidad <- 1 - ppois(8, lambda_4horas)
cat("c) P(X > 8 pacientes en 4 horas) =", round(prob_superar_capacidad, 4), "\n")
```
...
```{r}
capacidad_minima <- qpois(0.9, lambda_4horas)  
cat("d) Capacidad mínima para P(saturación) < 10%:", capacidad_minima, "pacientes\n")
```
...
verificar_tcl_poisson <- function(lambda, n_muestras, tamanos_muestra) {
    resultados <- list()
    
    for (n in tamanos_muestra) {
       
        muestras <- replicate(n_muestras, rpois(n, lambda))
        
        
        medias_muestrales <- apply(muestras, 2, mean)
        
        
        resultados[[as.character(n)]] <- medias_muestrales
    }
...
set.seed(123)
resultados_tcl <- verificar_tcl_poisson(lambda, n_muestras, tamanos_muestra)
...
for (i in seq_along(tamanos_muestra)) {
    n <- tamanos_muestra[i]
    medias <- resultados_tcl[[as.character(n)]]
    
    
    df <- data.frame(medias = medias)
    
  
    g <- ggplot(df, aes(x = medias)) +
        geom_histogram(aes(y = ..density..), bins = 30, fill = "#3498DB", alpha = 0.7) +
        stat_function(fun = dnorm, 
                     args = list(mean = media_teorica, 
                                sd = sqrt(varianza_teorica/n)),
                     color = "#E74C3C", size = 1) +
        labs(title = paste("Tamaño de muestra n =", n),
             x = "Media muestral",
             y = "Densidad") +
        theme_minimal() +
        xlim(media_teorica - 3*sqrt(varianza_teorica/n), 
             media_teorica + 3*sqrt(varianza_teorica/n))
...
grid.arrange(grobs = graficos, ncol = 2, 
             top = "Verificación del TCL para Poisson(λ = 7)")

cat("VERIFICACIÓN DEL TEOREMA CENTRAL DEL LÍMITE\n")
cat("Distribución: Poisson(λ =", lambda, ")\n")
...
for (n in tamanos_muestra) {
    ...
    cat("  Media muestral observada:", round(mean(medias), 4), "\n")
    cat("  Varianza muestral observada:", round(var(medias), 4), "\n")
    cat("  Varianza teórica (λ/n):", round(varianza_teorica/n, 4), "\n")
    cat("  Diferencia en medias:", round(abs(mean(medias) - media_teorica), 4), "\n")
    cat("  Diferencia en varianzas:", round(abs(var(medias) - varianza_teorica/n), 4), "\n\n")
}
...
cat("a) Probabilidad de degradación (capacidad actual):", round(prob_degradacion_actual, 6), "\n")
...
cat("b) Probabilidad de degradación (capacidad duplicada):", round(prob_degradacion_duplicada, 6), "\n")
```
...
cat("c) Capacidad óptima para P(degradación) < 1%:", capacidad_optima, "solicitudes\n")
cat("   Verificación - P(X >", capacidad_optima, ") =", round(1 - ppois(capacidad_optima, lambda), 6), "\n\n")
```
...
set.seed(666)
n_simulaciones <- 60
visitantes_por_minuto <- rpois(n_simulaciones, lambda)


degradacion_actual <- sum(visitantes_por_minuto > capacidad_actual)
degradacion_duplicada <- sum(visitantes_por_minuto > capacidad_duplicada)
degradacion_optima <- sum(visitantes_por_minuto > capacidad_optima)


porcentaje_degradacion_actual <- (degradacion_actual / n_simulaciones) * 100
porcentaje_degradacion_duplicada <- (degradacion_duplicada / n_simulaciones) * 100
porcentaje_degradacion_optima <- (degradacion_optima / n_simulaciones) * 100

cat("Parámetros:\n")
cat("  λ =", lambda, "visitantes por minuto\n")
cat("  Capacidad actual:", capacidad_actual, "solicitudes\n")
cat("  Capacidad duplicada:", capacidad_duplicada, "solicitudes\n\n")





cat("d) Simulación de 60 minutos:\n")
cat("   Minutos con degradación (capacidad actual):", degradacion_actual, "/ 60 =", round(porcentaje_degradacion_actual, 2), "%\n")
cat("   Minutos con degradación (capacidad duplicada):", degradacion_duplicada, "/ 60 =", round(porcentaje_degradacion_duplicada, 2), "%\n")
cat("   Minutos con degradación (capacidad óptima):", degradacion_optima, "/ 60 =", round(porcentaje_degradacion_optima, 2), "%\n\n")
