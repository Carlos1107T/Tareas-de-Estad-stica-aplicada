...
ggplot(calidad_data[1:8, ], aes(x = Defectos, y = Probabilidad)) +
  ...
  geom_vline(xintercept = media_defectos, color = "red", linetype = "dashed", lwd = 1) +
...
cat("a) Probabilidad de pieza no defectuosa:", 1-p_defectuosa, "\n")
...
```{r}
# Probabilidad de exactamente 2 defectuosas en 10 piezas (Binomial)
prob_2_defectuosas <- dbinom(2, size = 10, prob = p_defectuosa)
cat(" Probabilidad de exactamente 2 defectuosas en 10 piezas:", prob_2_defectuosas, "\n")
```
...
cat("Probabilidad de acertar una pregunta:", p_acierto, "\n")
```
...
k_caras <- 0:n_lanzamientos
prob_caras <- dbinom(k_caras, n_lanzamientos, p_cara)


moneda_data <- data.frame(
    Caras = k_caras,
    Probabilidad = prob_caras
)


moneda_filtrada <- moneda_data[moneda_data$Probabilidad > 0.001, ]

kable(moneda_filtrada,
    caption = paste("Distribución Binomial - Lanzamiento de moneda (n =", n_lanzamientos, ", p =", p_cara, ")"),
    digits = 4,
    col.names = c("Número de Caras", "Probabilidad")) %>%
kable_styling(bootstrap_options = "striped")
...
prob_8_caras <- dbinom(8, n_lanzamientos, p_cara)
cat(" Probabilidad de exactamente 8 caras:", prob_8_caras, "\n")
```
...
prob_al_menos_12 <- sum(dbinom(12:n_lanzamientos, n_lanzamientos, p_cara))
cat("Probabilidad de al menos 12 caras:", prob_al_menos_12, "\n")
```

c) Parámetros estadísticos
```{r}
 
media_moneda <- n_lanzamientos * p_cara
varianza_moneda <- n_lanzamientos * p_cara * (1 - p_cara)
desv_std_moneda <- sqrt(varianza_moneda)
...
k_curados <- 0:n_pacientes
prob_curados <- dbinom(k_curados, n_pacientes, p_efectividad)


medicamento_data <- data.frame(
    Curados = k_curados,
    Probabilidad = prob_curados
)


medicamento_filtrada <- medicamento_data[medicamento_data$Probabilidad > 0.001, ]

kable(medicamento_filtrada,
    caption = paste("Distribución Binomial - Efectividad medicamento (n =", n_pacientes, ", p =", p_efectividad, ")"),
    digits = 4,
...
cat("Probabilidad de exactamente 18 curados:", prob_18_curados, "\n")

```


# b) Probabilidad de entre 15 y 19 pacientes curados
```{r}

prob_15_a_19 <- sum(dbinom(15:19, n_pacientes, p_efectividad))
cat("Probabilidad de entre 15 y 19 curados:", prob_15_a_19, "\n")
```
...
cat("Número esperado de pacientes curados:", media_curados, "\n")
```
...
# Comparar con un binomial directa
W <- rbinom(1000, 25, 0.5)

datos_comparacion <- data.frame(
    Valor = c(Z, W),
    Tipo = c(rep("Suma de binomiales", 1000), rep("Binomial directa", 1000))
)
...
cat("Media de Z (X+Y):", media_Z, "\n")
cat("Media de W (Bin(25,0.5)):", media_W, "\n")
cat("Varianza de Z (X+Y):", varianza_Z, "\n")
cat("Varianza de W (Bin(25,0.5)):", varianza_W, "\n")


frecuencias_Z <- table(Z)/1000
frecuencias_W <- table(W)/1000

comparacion_frecuencias <- data.frame(
    Valor = as.numeric(names(frecuencias_Z)),
    Frecuencia_Suma = as.numeric(frecuencias_Z),
    Frecuencia_Binomial = as.numeric(frecuencias_W[match(names(frecuencias_Z), names(frecuencias_W))])
)

kable(comparacion_frecuencias,
    caption = "Comparación de frecuencias relativas - Suma vs Binomial directa",
    digits = 4,
    col.names = c("Valor", "Frecuencia Suma", "Frecuencia Binomial")) %>%
kable_styling(bootstrap_options = "striped")
