...
ggplot(hist_data) +
  ...
  labs(title = "Distribución de Medias Muestrales",
       subtitle = paste("n =", tamaño_muestra, "muestras de Uniforme(1,6)"),
       x = "Media Muestral",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
kable(urna_data,
    caption = "Distribución de probabilidad de la urna con 8 bolas",
    align = "cc") %>% 
    kable_styling(bootstrap_options = "striped")


numeros_pares <- seq(2, 8, by = 2)
prob_par <- sum(probabilidades[valores %in% numeros_pares])

cat("La probabilidad de obtener un número par es:", prob_par, "\n")
```
...
x2_n <- 15
valores3 <- x2_1:x2_n
probabilidades2 <- rep(1/(length(valores2)), length(valores3))
...
ggplot(comp_long, aes(x = Valor, y = Probabilidad, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Teórica" = "#126ABC", "Observada" = "darkred")) +
  labs(title = paste("Simulación con", n_sim, "observaciones"),
       x = "Valor",
       y = "Probabilidad",
       fill = "Distribución") +
