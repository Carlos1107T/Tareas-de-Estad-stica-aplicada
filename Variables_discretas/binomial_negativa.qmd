...
comparar_geometrica_binomial_negativa <- function(r, p, Nsim) {
    set.seed(42)
    
    # Método 1: Generar r variables geométricas y sumarlas
    suma_geometricas <- replicate(Nsim, {
        geometricas <- rgeom(r, prob = p)
        sum(geometricas)
    })
    
    # Método 2: Generar directamente de binomial negativa
    binomial_negativa <- rnbinom(Nsim, size = r, prob = p)
    ...
    stats_comparacion$Diff_Media_Teorica <- abs(stats_comparacion$Media - media_teorica)
    stats_comparacion$Diff_Var_Teorica <- abs(stats_comparacion$Varianza - var_teorica)
    
    
    data_comparacion <- data.frame(
        valores = c(suma_geometricas, binomial_negativa),
        metodo = rep(c("Suma de Geométricas", "Binomial Negativa"),
        each = Nsim)
    )
    ...
    grafico <- ggplot(data_comparacion, aes(x = valores, fill = metodo)) +
        ...
        labs(title = "Comparación: Suma de Geométricas vs Binomial Negativa",
             subtitle = paste("r =", r, ", p =", p, ", Nsim =", format(Nsim, big.mark = ",")),
             x = "Número de fracasos",
             y = "Frecuencia") +
        theme_minimal() +
        theme(legend.position = "none") +
...
# Ejemplo de uso
comparar_geometrica_binomial_negativa(r = 4, p = 0.3, Nsim = 10000)
```
...
prob_12_fallidas <- dnbinom(12, size = r_exitos, prob = p_exito)

cat("a) Probabilidad de exactamente 12 pruebas fallidas:", prob_12_fallidas, "\n")
...
```{r}
prob_mas_20_fallidas <- 1 - pnbinom(20, size = r_exitos, prob = p_exito)
cat("b) Probabilidad de más de 20 pruebas fallidas:", prob_mas_20_fallidas, "\n")
...
```{r}
fallidas_esperadas <- r_exitos * (1 - p_exito) / p_exito
costo_esperado_fallidas <- fallidas_esperadas * 10000
cat("c) Costo esperado en pruebas fallidas: $", costo_esperado_fallidas, "\n")
```
...
n_experimentos <- 10000
set.seed(123)
pruebas_fallidas <- replicate(n_experimentos, {
    sum(rgeom(r_exitos, prob = p_exito))
})


media_fallidas_sim <- mean(pruebas_fallidas)
prob_12_fallidas_sim <- mean(pruebas_fallidas == 12)
prob_mas_20_fallidas_sim <- mean(pruebas_fallidas > 20)
costo_esperado_sim <- media_fallidas_sim * 10000

cat("\nResultados de simulación (10,000 experimentos):\n")
cat("Media de fallidas simuladas:", media_fallidas_sim, "\n")
cat("Probabilidad de 12 fallidas (sim):", prob_12_fallidas_sim, "\n")
cat("Probabilidad de >20 fallidas (sim):", prob_mas_20_fallidas_sim, "\n")
cat("Costo esperado fallidas (sim): $", costo_esperado_sim, "\n")


laboratorio_data <- data.frame(
    Fallidas = 0:max(pruebas_fallidas),
    Probabilidad_Teorica = dnbinom(0:max(pruebas_fallidas), size = r_exitos, prob = p_exito)
)


tabla_probabilidades <- laboratorio_data[laboratorio_data$Fallidas <= 25, ]
kable(tabla_probabilidades,
    caption = "Distribución de probabilidad - Pruebas fallidas antes de 4 éxitos",
    digits = 6,
    col.names = c("Número de Fallidas", "Probabilidad Teórica")) %>%
    kable_styling(bootstrap_options = "striped")


data_simulacion <- data.frame(Fallidas = pruebas_fallidas)
grafico <- ggplot() +
    geom_histogram(data = data_simulacion, aes(x = Fallidas, y = ..density..),
                   bins = 30, fill = "#3498DB", alpha = 0.6) +
    geom_point(data = laboratorio_data, aes(x = Fallidas, y = Probabilidad_Teorica),
               color = "#E74C3C", size = 1.5) +
    geom_line(data = laboratorio_data, aes(x = Fallidas, y = Probabilidad_Teorica),
              color = "#E74C3C", linetype = "dashed") +
    labs(title = "Distribución de pruebas fallidas antes de 4 éxitos",
         subtitle = paste("p =", p_exito, ", r =", r_exitos, "| Histograma: simulación, Puntos: teórico"),
         x = "Número de pruebas fallidas",
         y = "Densidad/Probabilidad") +
    theme_minimal() +
    geom_vline(xintercept = fallidas_esperadas, linetype = "dashed", 
               color = "red", size = 1) +
    annotate("text", x = fallidas_esperadas + 2, y = max(laboratorio_data$Probabilidad_Teorica),
             label = paste("Media teórica =", round(fallidas_esperadas, 2)), color = "red")
