...
k_moneda <- 0:10
#Prefijo d (density) sufijo de la va
...
media_lanzamientos <- (1 - p_cara) / p_cara
...
moneda_geom <- data.frame(
    Lanzamientos = k_lanzamientos + 1,  # Ajustar porque dgeom(k) = P(X = k+1)
    Probabilidad = prob_lanzamientos
)

kable(moneda_geom,
    caption = "Distribución Geométrica - Primera cara en lanzamiento de moneda",
    digits = 4,
    col.names = c("Número de Lanzamientos", "Probabilidad")) %>%
kable_styling(bootstrap_options = "striped")
```
...
prob_exactamente_5 <- dgeom(4, p_cara)  # dgeom(k) = P(X = k+1)
cat("a) Probabilidad de necesitar exactamente 5 lanzamientos:", prob_exactamente_5, "\n")
```
...
prob_mas_de_3 <- 1 - sum(prob_lanzamientos[1:3]) 
cat("b) Probabilidad de necesitar más de 3 lanzamientos:", prob_mas_de_3, "\n")
```

    c)  Calcule la media y varianza del número de lanzamientos.
    
```{r}
cat("Lanzamiento de moneda - Primera cara:\n")
cat("Media:", media_lanzamientos, "\n")
...
media_intentos <- (1 - p_anotar) / p_anotar
varianza_intentos <- (1 - p_anotar) / (p_anotar^2)
desv_std_intentos <- sqrt(varianza_intentos)

cat("Tiro libre - Primera anotación:\n")
cat("Media:", media_intentos, "\n")
cat("Varianza:", varianza_intentos, "\n")
...
basquet_geom <- data.frame(
    Intentos = k_intentos + 1,  
    Probabilidad = prob_intentos
)


kable(basquet_geom,
    caption = "Distribución Geométrica - Primera anotación en tiro libre",
    digits = 4,
    col.names = c("Número de Intentos", "Probabilidad")) %>%
kable_styling(bootstrap_options = "striped")
```
...
prob_tercer_intento <- dgeom(2, p_anotar)  
cat("Probabilidad de anotar en el tercer intento por primera vez:", prob_tercer_intento, "\n")
```
...
prob_entre_2_5 <- sum(prob_intentos[1:4]) 
cat("c) Probabilidad de necesitar entre 2 y 5 intentos:", prob_entre_2_5, "\n")
```
...
media_inspecciones <- (1 - p_defectuosa) / p_defectuosa
varianza_inspecciones <- (1 - p_defectuosa) / (p_defectuosa^2)
desv_std_inspecciones <- sqrt(varianza_inspecciones)

cat("Inspección de piezas - Primera defectuosa:\n")
cat("Media:", media_inspecciones, "\n")
cat("Varianza:", varianza_inspecciones, "\n")


k_inspecciones <- 0:150
prob_inspecciones <- dgeom(k_inspecciones, p_defectuosa)
...
cat("a) Número esperado de inspecciones:", media_inspecciones + 1, "\n")
```
...
```{r}
prob_despues_100 <- 1 - sum(prob_inspecciones[1:100])  # P(X > 100)
cat("b) Probabilidad después de la pieza 100:", prob_despues_100, "\n")
```
...
```{r}
# c) Demostración de falta de memoria

s <- 50
t <- 30

# P(X > s + t | X > s) = P(X > s + t) / P(X > s)
prob_condicional <- (1 - sum(prob_inspecciones[1:(s+t)])) / (1 - sum(prob_inspecciones[1:s]))
prob_directa <- 1 - sum(prob_inspecciones[1:t])

cat("c) Demostración de falta de memoria:\n")
cat("   P(X >", s+t, "| X >", s, "):", prob_condicional, "\n")
cat("   P(X >", t, "):", prob_directa, "\n")
cat("   ¿Son iguales?", abs(prob_condicional - prob_directa) < 1e-10, "\n")


inspeccion_geom <- data.frame(
    Inspecciones = k_inspecciones + 1,
    Probabilidad = prob_inspecciones
)


kable(inspeccion_geom[1:10, ],
    caption = "Distribución Geométrica - Primera pieza defectuosa (primeras 10)",
    digits = 6,
    col.names = c("Número de Inspecciones", "Probabilidad")) %>%
kable_styling(bootstrap_options = "striped")
```
