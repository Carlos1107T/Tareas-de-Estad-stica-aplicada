...
La **distribución hipergeométrica** modela el número de éxitos al extraer una muestra sin reemplazo de una población finita que contiene dos tipos de objetos. Es fundamental en control de calidad, muestreo de aceptación y estudios donde el muestreo sin reemplazo afecta las probabilidades.

## 4.1 Definición y Función de Masa de Probabilidad

### Definición 4.1

Supongamos que tenemos $N$ objetos en total, de los cuales $K$ son de un primer tipo (éxitos) y $N - K$ son de un segundo tipo (fracasos). Se extrae una muestra de tamaño $n$ sin reemplazo. Sea $X$ el número de objetos del primer tipo en la muestra. Entonces $X$ tiene distribución hipergeométrica con parámetros $N$, $K$ y $n$, denotado $X \sim \text{hipergeo}(N, K, n)$.
...
#| fig-cap: "Función de Masa de Probabilidad Hipergeométrica"

library(ggplot2)

# Ejemplo del libro: N=20, K=7, n=5
N <- 20
K <- 7
n <- 5
...
## 4.2 Características de la Distribución Hipergeométrica
...
ggplot(df_bimodal, aes(x = factor(x), y = probabilidad, fill = tipo)) +
  ...
  labs(title = "Ejemplo de Distribución Bimodal",
       ...
       y = "Probabilidad") +
  theme_minimal() +
  scale_fill_manual(values = c("Moda" = "red", "Normal" = "steelblue"))
```

## 4.4 Teorema Central del Límite para la Distribución Hipergeométrica
...
donde se usa corrección por continuidad.

## 4.5 Simulación y Validación
...
La distribución hipergeométrica es esencial para modelar situaciones de muestreo sin reemplazo en poblaciones finitas. Su comprensión es crucial en áreas como control de calidad, estudios biológicos y cualquier contexto donde el muestreo afecte las probabilidades de manera significativa.

La distribución presenta propiedades matemáticas interesantes y converge a la distribución binomial cuando el tamaño de la población es grande en relación al tamaño de la muestra.